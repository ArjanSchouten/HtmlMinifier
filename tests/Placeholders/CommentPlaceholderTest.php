...
class CommentPlaceholderTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testConditionalCommentPlaceholder()
    {
        ...
        $result = $this->commentPlaceholder->process($context->setContents('<div>test</div><!--[if IE 6]>'.str_repeat(PHP_EOL,
                5).'<![endif]-->'));
        $this->assertEquals('<div>test</div>', $result->getContents());

        $result = $this->commentPlaceholder->process($context->setContents('<!--[if (gte IE 5.5)&(lt IE 7)]><p>You are using IE 5.5 or IE 6.</p><![endif]-->'));
        $this->assertEquals($placeholder.'<p>You are using IE 5.5 or IE 6.</p>'.$placeholder, $result->getContents());

        $result = $this->commentPlaceholder->process($context->setContents('<!--[if IE 6]>test -- test --<![endif]-->'));
        $this->assertEquals($placeholder.'test -- test --'.$placeholder, $result->getContents());
    }
    ...
    public function testNestedConditionalComments()
    {
        ...
        $this->assertEquals($placeholder.'<![if IE 7]><p>This nested comment is displayed in IE 7.</p><![endif]>'.$placeholder, $result->getContents());
    }
